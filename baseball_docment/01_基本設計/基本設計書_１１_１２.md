第11章 バッチ処理設計本システムは、サーバー側 (Laravel) で定期的に実行されるバッチ処理（スケジュールタスク）を定義する。11.1. バッチ処理の実装コード1. 試合ステータス自動更新バッチ試合日時を過ぎた「募集中」「満員」の試合を「開催済み」に自動更新する。ファイル: app/Console/Commands/UpdateGameStatus.php 1PHP<?php
...
2. プッシュ通知配信バッチ (試合リマインダー)試合開始1時間前の参加者に対して、リマインダープッシュ通知 (PUSH-003) を送信する。ファイル: app/Console/Commands/SendGameReminder.phpPHP<?php
...
11.2. cronスケジュール設定1. Laravelスケジューラーの設定app/Console/Kernel.php ファイルに、バッチ処理のスケジュールを定義する。PHP<?php
